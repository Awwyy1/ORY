...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const handleAddToCart = async () => {
    ...
    setIsAnimating(true);
    // Add to cart state immediately for the header pulse
    addToCart(product, selectedSize);

    // Sequence for local sophisticated animation
    setTimeout(() => {
  ...
  return (
    <div className="group relative">
      <div className="relative overflow-hidden aspect-[3/4] titanium-border rounded-sm bg-zinc-900">
        ...
        {/* Sophisticated Add Button Overlay */}
              ...
              {isAnimating ? (
                ...
                >
                  ...
                  <svg className="absolute inset-0 w-full h-full pointer-events-none" preserveAspectRatio="none">
                    ...
                    />
                    <motion.path
                      d="M -20 10 Q 80 40 150 10 T 280 10"
                      fill="transparent"
                      stroke="currentColor"
                      strokeWidth="1.5"
                      className="text-sky-400"
                      initial={{ pathLength: 0, x: -120 }}
                      animate={{ 
                        pathLength: [0, 1, 0], 
                        x: [0, 220],
